...
function applyEmailDimOrderLegend(row, body) {
  ...
  const parsed = findLegendDimInText(body, [L, W, H]);
...
/**
 * "N x N x N in (W x H x L)" (parens optional). When wantNums is set,
 * pick the legend whose three numbers match that multiset (any order) so
 * multi-line Comfortel RFQs remap each freight row correctly.
 * @param {string} text Body.
 * @param {Array<number>} [wantNums] Optional L/W/H values from the row.
 * @return {{length: number, width: number, height: number, labeled: boolean}|null}
 */
function findLegendDimInText(text, wantNums) {
  const re = new RegExp(
      "([\\d.]+)\\s*[x×*]\\s*([\\d.]+)\\s*[x×*]\\s*([\\d.]+)" +
      "\\s*(?:in(?:ches)?|cm|cms)?\\s*" +
      "\\(?\\s*" + AXIS_TOKEN + "\\s*[x×*]\\s*" + AXIS_TOKEN +
      "\\s*[x×*]\\s*" + AXIS_TOKEN + "\\s*\\)?",
      "gi");
  const want = Array.isArray(wantNums) ?
    wantNums.map(Number).filter((n) => n > 0).slice().sort((x, y) => x - y) :
    null;
  let m;
  let first = null;
  while ((m = re.exec(String(text || ""))) !== null) {
    const parsed = dimsFromAxisOrder(m[1], m[2], m[3], [
    ...
    if (!want || want.length !== 3) return parsed;
    const got = [parsed.length, parsed.width, parsed.height]
        .slice().sort((x, y) => x - y);
