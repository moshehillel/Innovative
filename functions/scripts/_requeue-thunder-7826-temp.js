"use strict";
/**
 * Requeue Thunder Funding Invoice #7826 / PO 266943 after NOA-cover fix.
 */
...
const MESSAGE_ID =
  "AAMkAGU5MDg4MmNlLTdjNGYtNGI2ZC1iYjMxLTA1YzI2ZmIwZTczMgBGAAAAAAAqntx80xUMS" +
    "q6IxxEc6_lyBwAKzphD0oUnQrPDK_2RRTFaAAAAAAEMAAAKzphD0oUnQrPDK_2RRTFaAAdBrc4cAAA=";
const QUEUE_URLS = [
...
async function waitForCompletion(messageId) {
  for (let i = 0; i < 36; i++) {
    ...
    console.log(`poll ${messageId.slice(-8)} ${i + 1}: ` +
      ...
      `summary=${String(q && q.summary || "").slice(0, 140)}`);
  ...
  console.log("final gmailQueue", messageId.slice(-8),
      JSON.stringify({
        ...
        itemSummaries: finalQ && finalQ.itemSummaries,
        invoiceId: finalQ && finalQ.invoiceId,
      }));
}

(async () => {
  console.log("=== requeue Thunder Funding Invoice #7826 / PO 266943 ===");
