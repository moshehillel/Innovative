"use strict";
/**
 * Requeue CSX Billing- 09/15/26 after AR-statement → Abe routing fix.
 */
...
const MESSAGE_ID =
  "AAMkAGU5MDg4MmNlLTdjNGYtNGI2ZC1iYjMxLTA1YzI2ZmIwZTczMgBGAAAAAAAqntx80xUMS" +
    "q6IxxEc6_lyBwAKzphD0oUnQrPDK_2RRTFaAAAAAAEMAAAKzphD0oUnQrPDK_2RRTFaAAdLY1o_AAA=";
const QUEUE_URLS = [
...
async function waitForCompletion(messageId) {
  for (let i = 0; i < 24; i++) {
    ...
    console.log(`poll ${messageId.slice(-8)} ${i + 1}: ` +
      ...
      `summary=${String(q && q.summary || "").slice(0, 160)}`);
    if (q && q.intakeStatus &&
  ...
  console.log("final gmailQueue", JSON.stringify({
    ...
    forwardReason: finalQ && finalQ.forwardReason,
  }));
}

(async () => {
  console.log("=== requeue CSX Billing 09/15/26 ===");
