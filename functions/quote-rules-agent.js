...
async function runQuoteRulesAgentTurn(opts) {
  ...
  const systemPrompt = [
    ...
    "4) carrier email note — match.carrierNameContains [roadrunner] +",
    "   notes text. Fires when that carrier is SELECTED into the",
    ...
    "For 'never add insurance' / strip Insurance even when requested:",
    "removeAccessorials [INS], match.flags [insuranceRequested],",
    "identifyVia both (or address_text). Runtime treats insuranceRequested",
    "as true when INS is on the lane or the email mentions insurance —",
    "do not invent a separate flags.insuranceRequested intake field.",
