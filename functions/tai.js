 ...
 * @return {Promise<void>}
 */
async function logWebhook(level, message, details = {}) {
  try {
    const clean = JSON.parse(JSON.stringify(details, (key, value) => {
      return value === undefined ? null : value;
    }));
    await db().collection("taiWebhookLogs").add({
      level,
      message,
      details: clean,
 ...
 */
async function indexShipment(shipment, eventType) {
  ...
  const shipmentRef = db().collection("taiShipments").doc(String(shipmentId));
  ...
  for (const entry of entries) {
    const lookupRef = db().collection("taiShipmentLookups").doc(entry.key);
    batch.set(lookupRef, {
...
exports.taiWebhook = onRequest(
    ...
    async (req, res) => {
      ...
      if (!isAuthorized(req)) {
        await logWebhook("warn", "Rejected unauthorized webhook", {eventType});
      ...
      try {
        const result = await indexShipment(shipment, eventType);
 ...
 *   matchedBy, snapshot.
 */
async function resolveTaiShipmentId({loadNumber, proNumber} = {}) {
  const candidates = [
...
exports.taiResolveShipment = onRequest(
    ...
    async (req, res) => {
      ...
      try {
        const result = await resolveTaiShipmentId({loadNumber, proNumber});
 ...
 */
async function loadSnapshot(shipmentId) {
  const snap = await db()
      .collection("taiShipments")
      .doc(String(shipmentId))
      .get();
...
exports.processTaiWorkflow = onRequest(
    ...
    async (req, res) => {
      ...
      try {
        ...
        const snapshot = await loadSnapshot(shipmentId);
